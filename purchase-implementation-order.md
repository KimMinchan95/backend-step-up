...
- [ ] 상품 ID와 구매 수량을 받아 재고를 차감하는 메서드를 추가한다.
- [ ] 여러 파라미터에 `@Param`을 지정하고 XML의 이름과 맞춘다.
- [ ] 재고가 구매 수량 이상일 때만 차감하는 UPDATE를 작성한다.
- [ ] 조회한 재고를 Java에서 계산해 덮어쓰지 않고, SQL에서 현재 재고를 차감한다.
- [ ] 반환값으로 변경 행 수를 받는다. 1이면 성공, 0이면 상품 없음 또는 재고 부족이다.
- [ ] DB 콘솔에서 충분한 재고, 재고와 같은 수량, 부족한 재고로 UPDATE를 직접 실행한다.
- [ ] 실행 전후의 재고와 변경 행 수를 확인한다.
