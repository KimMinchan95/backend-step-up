...
- [x] 콜라의 재고가 10개이고 구매내역이 0건인지 확인한다.
- [x] `POST /purchases`에 다음 요청을 보낸다.

```json
{ "productId": 1, "quantity": 2 }
```

- [x] 201 성공 응답에 구매 ID, 상품 ID 1, 수량 2, 개당 가격 1500원, 구매 시각이 있는지 확인한다.
- [x] DB에서 콜라 재고가 8개인지 확인한다.
- [x] 구매내역이 1건이고, 상품 ID는 1, 수량은 2, 개당 가격은 1500원인지 확인한다.
- [x] 응답의 구매 ID로 DB를 조회해서 응답 값이 저장된 구매내역과 일치하는지 확인한다.

### 구매내역 저장 실패와 롤백을 확인한다

- [x] MySQL을 다시 초기화해서 콜라 재고 10개, 구매내역 0건으로 시작한다.
- [x] 로컬 실습 중에만, 재고 차감 후 구매내역 객체에 넣는 수량을 잠시 0으로 바꾼다.
- [x] 앱에 변경 코드를 반영하고, 위와 같은 정상 구매 수량 2의 HTTP 요청을 보낸다.
- [x] 재고 차감 뒤 구매내역 INSERT가 `CHECK (quantity > 0)` 제약조건으로 실패하는지 확인한다.
- [x] 저장 예외를 잡아서 성공으로 처리하지 않고, 구매 Service 밖으로 전달한다.
- [x] 구매 요청에 기본 500 오류 응답이 오는지 확인한다. 구체적인 실패 원인은 서버 로그에서 확인한다.
- [x] DB 콘솔에서 새로 SELECT해서 콜라 재고가 10개이고 구매내역이 0건인지 확인한다.
- [x] 확인이 끝나면 구매내역에 정상 수량을 넣도록 코드를 원래대로 돌린다.
